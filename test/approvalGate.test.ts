...
describe("ApprovalGate", () => {
  it("reject reports whether a pending proposal actually existed", async () => {
    ...
    await gate.propose("id-1", { domain: "work", summary: "test", kind: "run_script" });
  ...
  it("approve returns the request once, then null on re-approval", async () => {
    const gate = new ApprovalGate(fakeNotifier());
    await gate.propose("id-2", { domain: "personal", summary: "test", kind: "run_script" });

    expect(gate.approve("id-2")?.domain).toBe("personal");
  ...
  it("propose notifies exactly once per proposal", async () => {
    ...
    await gate.propose("id-3", { domain: "work", summary: "test", kind: "run_script" });
